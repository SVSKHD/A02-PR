...
History (one line per behavioral change):
  ...
  3.3.4  RALLY PULLBACK DETECTOR (rally boosts only) -- an entry-relative early-cut
         ...
         detector (rally-only; boost_kind gate). Config knobs (NUMBERS TBD FROM LIVE
         DATA -- starting defaults, fully tunable): rally_pullback_enabled=True (ship
         ON), rally_pullback_tol_dollars=13.0 (T; clamped to the backstop -- at $13 the
         distance cut coincides with the backstop so only the time bound adds new
         behavior; drop toward the $7-8 candidate once measured),
         rally_pullback_time_bound_min=30.0 (B; 0 disables). Position gains a
         pullback_since field (when the current pullback began; None outside one). No
         rally trail number changed (fire +$5, trail peak-$2, +$3 floor, one-way) and
         RESCUE is byte-identical. selftest -> 95 steps (new 94 pullback band:
         hold-within-T / cut-beyond-T / gap-floored-at-backstop / rescue-unaffected;
         95 recover/time: recovery resets, B-min slow-reversal cut, default T=13 ==
         backstop no-early-cut). Banner v3.3.4.
"""
