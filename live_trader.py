...
from trails import TrailsMixin
...
class LiveTrader(StateMixin, RiskMixin, AnchorsMixin, FillsMixin, TrailsMixin, JournalMixin):
    """
    ...
    def __init__(self, cfg, adapter, paper: bool = True):
        from strategy import Position, anchor_datetime_utc, eod_datetime_utc  # late import
    ...
    def run(self):
        ...
        # v3.0.0: module receipt — list the AUREON modules actually imported so a
        # stale file on the VPS (deployment drift) is visible in Telegram.
        import sys as _sys
        _aureon_mods = [m for m in (
            'config', 'strategy', 'mt5_adapter', 'anchors', 'fills', 'trails',
            'risk', 'journal', 'state', 'firebase_journal', 'telemetry',
            'env_loader', 'watchdog', 'version', 'bot', 'live_trader',
        ) if m in _sys.modules]
        module_receipt = "\nModules `v3.0.0`: " + ", ".join(_aureon_mods)
        self.tele.success(
        ...
        try:
            ...
            if tick_age_sec > 3600:
                ...
                market_open = False
                # v3.0.0: market closed (e.g. Sunday startup) — reconcile the
                # week's journal to Firebase before idling. Fail-safe.
    ...
    def _tick(self):
        ...
        if self._eod_reached(broker_date, utc_now):
            ...
            # v3.0.0: Firebase EOD journal — once per broker day, after the
            # flatten so the day's P&L is final. Fail-safe: guarded here AND
            # inside firebase_journal; never blocks trading or the flatten.
            if self.state.get("firebase_eod_done_for") != str(broker_date):
                self._firebase_eod_save(broker_date)
                self.state["firebase_eod_done_for"] = str(broker_date)
                self._save_state()
            if self._tick_counter % self.STATUS_EVERY_TICKS == 0:
                self._write_status(broker_date)
            return
