"""
AUREON — core strategy engine. THE most precious code in the repo.

PURE logic, no I/O. Position dataclass + update_position_on_bar (the bar-by-bar
exit/trail/ladder rule) + the small price helpers and anchor-time scheduling
functions. Shared byte-identically between the backtest (bot.py) and the live
trail manager (trails.py). Behavior is FROZEN at v2.9.8 — do not edit logic here.

Extracted verbatim from bot.py in the v3.0.0 structural refactor.
"""
